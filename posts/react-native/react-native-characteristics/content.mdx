...
JavaScript로 `Core Components`는 네이티브 컴포넌트에 직접 매핑이 되고, 네이티브 모듈은 `Bridge`를 통해 사용할 수 있기 때문에 GPS, Bluetooth, Camera 등등 사용자 경험을 해당 OS에 맞게 제공할 수 있다는 점이 인상 깊었습니다.
...
앱 개발자들은 매번 **스토어 심사**라는 것에 시달리며 살 거라고 확신합니다. 웹의 배포 프로세스 사이에 구글과 애플이 감시를 받아야 한다는 거죠.
